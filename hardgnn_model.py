...
class Recommender:
	...
	def sampleTrainBatch(self, batIds, labelMat, timeMat, train_sample_num):
		...
		# Prepare sequences and masks first
		...
		feed_dict = {
			...
			self.is_train: False,
			self.keepRate: 1.0,
			self.uids: np.zeros(2, dtype=np.int32),
			self.iids: np.zeros(2, dtype=np.int32),
			self.uLocs_seq: np.zeros(2, dtype=np.int32),
		}
		for k in range(args.graphNum):
			feed_dict[self.suids[k]] = np.zeros(2, dtype=np.int32)
			feed_dict[self.siids[k]] = np.zeros(2, dtype=np.int32)
			feed_dict[self.suLocs_seq[k]] = np.zeros(2, dtype=np.int32)

		user_embeddings, item_embeddings = self.sess.run([self.att_user, self.final_item_vector], feed_dict=feed_dict)

		# Prepare user interaction lists and positive samples
		poslocs_batch = []
		interactions_batch = []
		choose_vals = []
		...
		for i in range(batch):
			...
			choose = 1
			if sampNum == 0:
				poslocs_batch.append([np.random.choice(args.item)])
				interactions_batch.append(temLabel[i])
			else:
				choose = randint(1, max(min(args.pred_num + 1, len(posset) - 3), 1))
				poslocs_batch.append([posset[-choose]] * sampNum)
				interactions_batch.append(temLabel[i])
			choose_vals.append(choose)

		# Use FAISS for batched hard negative sampling
		if args.use_hard_neg:
			neg_batch = self.sample_batch_hard_negatives(
				user_embeddings[:batch], item_embeddings, interactions_batch, train_sample_num
			)
		else:
			neg_batch = [
				negSamp(interactions_batch[i], train_sample_num, args.item,
						[self.handler.sequence[batIds[i]][-1], temTst[i]], self.handler.item_with_pop)
				for i in range(batch)
			]

		# Fill final training triplets
		for i in range(batch):
			sampNum = len(poslocs_batch[i])
			for j in range(sampNum):
				posloc = poslocs_batch[i][j]
				negloc = neg_batch[i][j]
	...
	# 	"""
	# 	Sample hard negatives for a given user using FAISS over a 300-item subset.
	...
	# 	"""
	# 	print('starting hard neg sampling')
	...
	# 		user_embed = user_att[0].astype('float32').reshape(1, -1)
	# 		items_embed = items_embed.astype('float32')

	# 		# Subset: choose 300 candidate negatives
	# 		subset_size = 300
	# 		subset_indices = np.random.choice(len(items_embed), size=subset_size, replace=False)
	# 		subset_embeddings = items_embed[subset_indices]

	# 		# Normalize
	# 		faiss.normalize_L2(subset_embeddings)
	# 		faiss.normalize_L2(user_embed)

	# 		index = faiss.IndexFlatIP(subset_embeddings.shape[1])
	# 		index.add(subset_embeddings)

	# 		_, top_indices = index.search(user_embed, sampNum * 10)

	# 		hard_negatives = [subset_indices[i] for i in top_indices[0] if user_interactions[subset_indices[i]] == 0][:sampNum]
	# 		print('done')
	# 		return hard_negatives

	# 	except Exception as e:
	# 		print("FAISS hard negative sampling failed, falling back to random. Error:", e)
	# 		return negSamp(user_interactions, sampNum, args.item,
	# 			[self.handler.sequence[user_id][-1] if len(self.handler.sequence[user_id]) > 0 else None,
	# 			self.handler.tstInt[user_id]], self.handler.item_with_pop)
	...
	def sample_batch_hard_negatives(self, user_embeddings, item_embeddings, user_interactions_batch, sampNum):
		"""
		Sample hard negatives for a batch of users using FAISS.
		
		Args:
			user_embeddings: (batch_size, emb_dim)
			item_embeddings: (n_items, emb_dim)
			user_interactions_batch: list of binary arrays for each user
			sampNum: number of negatives to sample per user
		...
		# Subsample items
		subset_indices = np.random.choice(len(item_embeddings), size=subset_size, replace=False)
		subset_embeddings = item_embeddings[subset_indices]
		faiss.normalize_L2(subset_embeddings)
		...
		_, top_indices = index.search(user_embeddings, sampNum * 10)

		# Filter out positives per user
		hard_negatives_batch = []
		for i in range(batch_size):
			hard = [subset_indices[j] for j in top_indices[i] if user_interactions_batch[i][subset_indices[j]] == 0][:sampNum]
			hard_negatives_batch.append(hard)
