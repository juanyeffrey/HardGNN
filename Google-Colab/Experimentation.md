...
3.  [Running Experiments](#running-experiments)
    *   [Option A: Using the Automated Colab Script (Recommended)](#option-a-using-the-automated-colab-script-recommended-for-full-runs)
    *   [Option B: Manual Execution / Interactive Mode](#option-b-manual-execution--interactive-mode)
...
### Option B: Manual Execution / Interactive Mode
This approach is suitable if you want to run steps individually, modify parameters frequently, or debug parts of the code. You can use this in conjunction with the `HardGNN_Jupyter_Notebook_Guide.md` for a cell-by-cell experience.
...
Model and hard negative sampling parameters are primarily managed in `Params.py`. You can edit this file directly through the Colab file browser if you need to make persistent changes.

For dataset-specific configurations when running `main.py` directly, you can either:
*   Modify `main.py` to call `configure_dataset_ напрямую()` similar to how `HardGNN_Colab_Script.py` does.
*   Or, pass the dataset as a command-line argument as shown in Step B4.
...
```
The script will then load the appropriate dataset configuration from `Params.py` (which `main.py` updates based on the `--dataset` argument) and proceed with training.
...
The core HNS parameters are defined in `Params.py` and can be modified there before running any script. If `HardGNN_Colab_Script.py` is used, it might override some `Params.py` settings based on its internal logic, so ensure consistency or modify the script as needed.
...
3.  Modify their default values or how they are set.
4.  Save the file.
...
*   **`HardGNN_Jupyter_Notebook_Guide.md`**: This guide provides a detailed breakdown of how to run the `HardGNN_Colab_Script.py` content cell-by-cell in a Jupyter Notebook. This is excellent for interactive experimentation and understanding each part of the pipeline.
